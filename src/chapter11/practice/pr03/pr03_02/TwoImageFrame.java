package chapter11.practice.pr03.pr03_02; 			// chapter11.practice.pr03.pr03_02 패키지에 속하는 클래스임을 명시
...
public class TwoImageFrame extends JFrame {
    ...
    public TwoImageFrame() {
        setTitle("두 이미지 밑으로 나란히 보이기"); // 프레임의 제목 설정
    ...
    class MyPanel extends JPanel {
        private Image imgA = new ImageIcon("images/practice/11-03-02/a.jpg").getImage(); // 첫 번째 이미지 로딩
        private Image imgB = new ImageIcon("images/practice/11-03-02/b.jpg").getImage(); // 두 번째 이미지 로딩

        // paintComponent 메서드 오버라이딩
        public void paintComponent(Graphics g) {
            super.paintComponent(g); // 부모 클래스(JPanel)의 paintComponent 메서드 호출
            
            // 첫 번째 이미지를 패널의 위쪽 절반에 그린다.
            g.drawImage(imgA, 0, 0, getWidth(), getHeight() / 2, this);
            
            // 두 번째 이미지를 패널의 아래쪽 절반에 그린다.
